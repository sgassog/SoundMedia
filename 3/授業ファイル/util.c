...
void wave_write_IMA_ADPCM_mono(MONO_PCM *pcm, char *file_name)
{
    ...
    channel = 1;
    samples_per_sec = pcm->fs; /* 標本化周波数 */
    bytes_per_sec = (int)(block_size * samples_per_sec / samples_per_block);
...
void triangle_wave_sin(MONO_PCM *pcm, double f0, int offset, int duration)
{
    ...
    s = (double *) calloc(duration, sizeof(double));
...
void square_wave_sin(MONO_PCM *pcm, double f0, int offset, int duration)
{
    ...
    for (i = 1; i <= 100; i = i + 2) {
        for (n = 0; n < duration; n++) {
            s[n] += 1.0 / i * sin(2.0 * M_PI * i * f0 * n / pcm->fs);
        }
    }
...
/* 三角波、矩形波、ノコギリ波を１つ選択して波形生成 */
void make_wave(WAVE_TYPE wave_type, MONO_PCM *pcm, double f0, double gain,
               int offset, int duration)
{
    switch (wave_type) {
    case TRIANGLE:
        triangle_wave(pcm, f0, gain, offset, duration);
        break;
    case RECTANGLE:
        square_wave(pcm, f0, gain, offset, duration);
        break;
    case SAW:
        saw_wave(pcm, f0, gain, offset, duration);
        break;
    default:
        break;
    }
...
/* 三角波、矩形波、ノコギリ波を１つ選択して和音生成 */
void make_chord(WAVE_TYPE wave_type, MONO_PCM *pcm, char *chord, double gain,
                int offset, int duration)
{
    if (strcmp(chord, "C")  == 0) {
        make_wave(wave_type, pcm, C4, gain, offset, duration);
        make_wave(wave_type, pcm, E4, gain, offset, duration);
        make_wave(wave_type, pcm, G4, gain, offset, duration);
    } else if (strcmp(chord, "Dm")  == 0) {
        make_wave(wave_type, pcm, D4, gain, offset, duration);
        make_wave(wave_type, pcm, F4, gain, offset, duration);
        make_wave(wave_type, pcm, A4, gain, offset, duration);
    } else if (strcmp(chord, "Em")  == 0) {
        make_wave(wave_type, pcm, E4, gain, offset, duration);
        make_wave(wave_type, pcm, G4, gain, offset, duration);
        make_wave(wave_type, pcm, B4, gain, offset, duration);
    } else if (strcmp(chord, "F")  == 0) {
        make_wave(wave_type, pcm, F4, gain, offset, duration);
        make_wave(wave_type, pcm, A4, gain, offset, duration);
        make_wave(wave_type, pcm, C5, gain, offset, duration);
    } else if (strcmp(chord, "G")  == 0) {
        /* どちらでも可 (B3, D4, G4) or (G4, B4, D5) */
        /* 「きらきらぼし」については (B3, D4, G4) のほうがいい */
        make_wave(wave_type, pcm, B3, gain, offset, duration);
        make_wave(wave_type, pcm, D4, gain, offset, duration);
        make_wave(wave_type, pcm, G4, gain, offset, duration);
        // make_wave(wave_type, pcm, G4, offset, duration);
        // make_wave(wave_type, pcm, B4, offset, duration);
        // make_wave(wave_type, pcm, D5, offset, duration);
    } else if (strcmp(chord, "Am")  == 0) {
        make_wave(wave_type, pcm, A4, gain, offset, duration);
        make_wave(wave_type, pcm, C5, gain, offset, duration);
        make_wave(wave_type, pcm, E5, gain, offset, duration);
    }
}
                     ...
                     double gain, int offset, int duration)
{
    if (strcmp(chord, "C")  == 0) {
        ...
        make_wave(wave_type, pcm, G4, gain, offset, duration);
        make_wave(wave_type, pcm, B4, gain, offset, duration);
        make_wave(wave_type, pcm, D5, gain, offset, duration);
    } else if (strcmp(chord, "Am")  == 0) {
        make_wave(wave_type, pcm, A4, gain, offset, duration);
        make_wave(wave_type, pcm, C5, gain, offset, duration);
        make_wave(wave_type, pcm, E5, gain, offset, duration);
